...
### Passing Durable Object storage to Dynamic Workers

Dynamic Workers do not have their own persistent storage — they run user-provided code that exists only for the duration of execution. If that code needs to store data between runs, you can give it access to Durable Object storage by passing a storage interface via RPC.

You can do this by wrapping [Durable Object storage](/durable-objects/api/sqlite-storage-api/) in an [`RpcTarget`](/workers/runtime-apis/rpc/) and passing it to the Dynamic Worker. This allows the Dynamic Worker to call methods like `storage.put()` and `storage.get()` without direct access to your Durable Object. You can implement the storage interface using the [KV API](/durable-objects/api/sqlite-storage-api/#synchronous-kv-api), or use [SQLite](/durable-objects/api/sqlite-storage-api/#sql-api) for more complex queries (as the example below shows).

#### When to use `env` vs function parameters

When passing bindings or resources to a Dynamic Worker, choose the method based on the resource's lifecycle:

- **[`env`](#env):** Use for global resources like KV namespaces, R2 buckets, or D1 databases. These are independent of the caller and remain valid for the entire lifespan of the Worker.
- **Function parameters:** Use for resources that belong to the Durable Object instance making the call, such as a storage wrapper referencing `this.ctx.storage`. This ensures the Durable Object stays alive and connected to provide access to its data for the duration of the call.
...
export class MyDurableObject extends DurableObject<Env> {
  ...
  async call(request: Request) {
    ...
    return await worker.getEntrypoint().call(request, new StorageContext(this.ctx.storage));
  }
}
