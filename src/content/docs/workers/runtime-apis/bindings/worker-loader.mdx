...
When passing bindings or resources to a Dynamic Worker, choose the method based on what you're passing:

- **[`env`](#env):** Use for global resources like [KV namespaces](/kv/), [R2 buckets](/r2/), or [D1 databases](/d1/). These are independent of the caller and remain valid for the entire lifespan of the Worker.
- **Function parameters:** Use when you need to give a Dynamic Worker access to a Durable Object's storage or other instance-specific state. Passing these as function parameters keeps the Durable Object alive and connected for the duration of the call.
