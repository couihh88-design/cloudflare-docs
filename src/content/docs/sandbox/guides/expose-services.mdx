...
By default, preview URLs change each time you expose a port because a new random token is generated. For consistent URLs across sandbox restarts, use custom tokens:
...
console.log('API URL:', api.exposedAt);
// https://8080-sandbox-id-stable_api_v1.yourdomain.com

// Without custom token - URL changes each restart
const dev = await sandbox.exposePort(3000, { hostname, name: 'dev' });
// https://3000-sandbox-id-random_token.yourdomain.com
```
</TypeScriptExample>
...
- **Stable demo URLs** - Share consistent links with stakeholders
- **Webhook endpoints** - Configure third-party services with permanent URLs
- **Development environments** - Maintain consistent URLs across restarts
...
- **Use custom tokens** - For consistent URLs across restarts when needed
