...
- `options`:
  ...
  - `token` - Custom token for preview URL (optional, 1-16 characters: a-z, 0-9, _)
...
// With custom token - URL stays consistent across restarts
const stable = await sandbox.exposePort(8080, { 
  hostname, 
  token: 'my_token_v1' // 1-16 chars: a-z, 0-9, _
});
console.log(stable.exposedAt);
// https://8080-sandbox-id-my_token_v1.yourdomain.com
```
</TypeScriptExample>
...
By default, preview URLs use randomly generated tokens that change each time you expose a port. For consistent URLs across sandbox restarts, use custom tokens:
...
// Without token - URL changes each time
const { url: randomUrl } = await sandbox.exposePort(8080, {
  hostname: 'example.com'
});
...
**Token requirements:**
- 1-16 characters containing only lowercase letters, numbers, and underscores
