...
- Port 8080: `https://8080-abc123-random_token.yourdomain.com`
- Port 3000: `https://3000-abc123-custom_token.yourdomain.com`

**Local development**: `http://localhost:8787/...`

Preview URLs remain stable while a port is exposed and can be shared during that time. However, if you unexpose and re-expose a port, a new random token is generated and the URL changes. For persistent URLs, you can either keep ports exposed for the duration you need them accessible, or use custom tokens.

## Custom Tokens

For consistent URLs across sandbox restarts, you can specify custom tokens when exposing ports:

```typescript
// Custom token - URL stays the same
const exposed = await sandbox.exposePort(8080, {
  hostname,
  token: 'my_stable_token'
});
// https://8080-abc123-my_stable_token.yourdomain.com

// Auto-generated token - URL changes each time
const exposed = await sandbox.exposePort(8080, { hostname });
// https://8080-abc123-random_token_here.yourdomain.com
```

**Token requirements:**
- 1-16 characters containing only lowercase letters (a-z), numbers (0-9), and underscores (_)
...
- **Token-based access** - Each exposed port gets a unique token in the URL (for example, `https://8080-sandbox-abc123_token.yourdomain.com`)
